...
{ unsigned i;
  ...
  f= fopen (KD_KDApplication::GetApplication()->GetArtFile("survival.sco").c_str(), "r");
  if (f!= NULL)
  { signed res= hst[0]->LoadTable (f);
    fclose (f);
    if (res!= 0) /* the high scores table is corrupt */
...
{
#ifndef NO_MUSIC
    music->Load(KD_KDApplication::GetApplication()->GetArtFile(MUSIC_NAME[KD_MUS_HIGHSCORES]).c_str());
    music->PlayMusic();
#endif
